...
const readGoogleSheet = async () => {
    const { GSHEET_ID, GSHEET_NAME, GSHEET_RANGE } = process.env
...
const registerUserSession = async (chatID, username,session) => {
    ...
    try {
        gSheetClient.spreadsheets.values.append({
            spreadsheetId: GSHEET_ID,
            range: 'Users!A:D',
...
const published = async () => {
    ...
    const gSheetClient = await getGSheetClient()
    gSheetClient.spreadsheets.values.update({
        spreadsheetId: GSHEET_ID,
        range: 'Settings!A2',
        valueInputOption: 'RAW',
        resource: {
